...
const debouncedSyncTasksToDrive = () => {
  ...
  driveSyncTimer = setTimeout(async () => {
    try {
      const { syncTasksToDrive } = await import('@/utils/googleDriveSync');
      await syncTasksToDrive();
    } catch (e) {
      console.warn('Auto-upload tasks to Drive failed:', e);
    }
  }, DRIVE_SYNC_DEBOUNCE);
};
...
export const saveTodoItems = async (
  ...
  try {
    await saveTasksToDB(migrated);
    // Auto-upload to Google Drive (debounced)
